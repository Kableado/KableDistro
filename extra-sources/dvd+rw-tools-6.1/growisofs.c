/*
 * growisofs 6.1 by Andy Polyakov <appro@fy.chalmers.se>.
 *
...
{ double		ratio,velocity,slept;
  ...
  sigset_t		set;
	...
	{   delta = time (NULL) - progress.zero;
	    ...
	    fprintf (stdout,"%10"LLD"/%"LLD" (%4.1f%%) @%.1fx, "
			    "remaining %d:%02d "
	...
	{   rbu = (int)((current - outoff)/DVD_BLOCK);
	    rbu = highest_ecc_block - rbu;
	    fprintf (stdout,"%10"LLD"/%"LLD" (%4.1f%%) @0x, "
			    "remaining ??:?? "
			    "RBU %.1f%%\n",	/* ring buffer utilization */
...
#undef CLOSEONEXEC
	 ...
	 */
	for (rval=3;rval>=0;rval--)	/* reuse rval as retry count */
	{   /*
